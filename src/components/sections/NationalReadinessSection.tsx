  ...
  return (
    <section ref={ref} id="implementation" className="relative py-32 overflow-hidden" style={{ background: "#F2EDE4" }}>
      <div className="section-inner">
          ...
          <div>
            ...
            <ScrollReveal progress={scrollYProgress} start={0.09} end={0.26} direction="up" distance={40} fadeOut={false}>
              <h2 style={{ fontSize: "clamp(2rem,3.8vw,3rem)", color: "var(--navy)" }} className="mb-3">
                Building the <span className="text-gold-gradient">National Readiness</span><br />System for the AI Era
              </h2>
            </ScrollReveal>
        ...
        <ScrollReveal progress={scrollYProgress} start={0.38} end={0.5} direction="up" fadeOut={false}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {pillars.map((p, i) => (
            <ScrollRevealItem key={p.title} progress={scrollYProgress} index={i} baseStart={0.45} step={0.045} direction="up" distance={36} fadeOut={false}>
              <Card className="relative h-full p-5 cursor-default overflow-hidden">
                ...
                <h3 className="text-[11px] font-bold mb-2 uppercase tracking-wide leading-snug" style={{ color: "var(--navy)" }}>{p.title}</h3>
                <p className="text-[10px] leading-relaxed" style={{ color: "var(--text-muted)" }}>{p.desc}</p>
                <div className="absolute bottom-0 left-0 h-[2px]"
                  style={{ width: `${(i + 1) * 20}%`, background: "linear-gradient(to right,#B8962E,#D4AF5A)", opacity: 0.38 }} />
              </Card>
            </ScrollRevealItem>
          ))}
        </div>
        ...
        {/* Banner */}
        <ScrollReveal progress={scrollYProgress} start={0.72} end={0.84} direction="up" fadeOut={false}>
          <div className="mt-12 p-8 text-center border" style={{ background: "rgba(255,255,255,0.5)", borderColor: "rgba(184,150,46,0.15)" }}>
