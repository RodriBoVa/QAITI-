    ...
    <section ref={ref} id="teacher-lab" className="relative py-32 overflow-hidden" style={{ background: "var(--cream)" }}>
      <div className="section-inner">
        <div className="grid lg:grid-cols-2 gap-20 items-start mb-20">
          ...
          <div>
            ...
            <ScrollReveal progress={scrollYProgress} start={0.19} end={0.34} direction="up" fadeOut={false}>
              <p className="text-[0.9rem] leading-relaxed mb-4" style={{ color: "var(--text-muted)" }}>
            ...
            <div className="relative">
              ...
              >
                {[{ val: "150+", label: "Resources Available" }, { val: "3,250+", label: "Teachers Connected" }].map((s) => (
                  <div key={s.label} className="p-4 text-center" style={{ background: "rgba(250,248,244,0.93)", backdropFilter: "blur(12px)" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {features.map((f, i) => (
            <ScrollRevealItem key={f.title} progress={scrollYProgress} index={i} baseStart={0.5} step={0.04} direction="up" distance={36} fadeOut={false}>
              <Card className="h-full flex flex-col text-center items-center p-5 cursor-default" style={{ background: "rgba(255,255,255,0.65)" }}>
                <h3 className="text-[10.5px] font-bold mb-2 uppercase tracking-wide leading-snug" style={{ color: "var(--navy)" }}>{f.title}</h3>
        ...
        {/* Bottom banner */}
        <ScrollReveal progress={scrollYProgress} start={0.74} end={0.85} direction="up" fadeOut={false}>
          <div className="mt-12 p-6 text-center" style={{ background: "var(--navy)" }}>
            <p className="text-sm" style={{ color: "rgba(255,255,255,0.72)" }}>
              When teachers are empowered, students thrive.{" "}
              <span style={{ color: "var(--gold)" }}>Teacher Intelligence Lab™ — Enabling teachers. Elevating education. Shaping the future.</span>
            </p>
          </div>
        </ScrollReveal>
      </div>
